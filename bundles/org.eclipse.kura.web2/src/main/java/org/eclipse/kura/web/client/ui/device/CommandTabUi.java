...
public class CommandTabUi extends Composite {
    ...
    public CommandTabUi() {
        ...
        this.commandForm.addSubmitCompleteHandler(event -> {
            ...
            // the servlet leaves the body empty when the upload succeeds and reports the reason otherwise
            final String result = event.getResults() == null ? "" : event.getResults().trim();

            if (result.isEmpty()) {
                callback.onSuccess(null);
            } else if (result.contains("HTTP ERROR")) {
                callback.onFailure(new IllegalStateException(MSGS.fileUploadFailure()));
            } else {
                callback.onFailure(new IllegalStateException(GwtSafeHtmlUtils.htmlUnescape(result)));
            }
