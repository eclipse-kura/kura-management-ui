...
public class FileServlet extends AuditServlet {
    ...
    private static final String COMMAND_ENABLE = "command.enable";
    ...
    private void doPostCommand(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (isNull(commandServiceReference)
            ...
            reportCommandFailure(resp, "The command service is not enabled");
            return;
        }
        ...
        try {
            ...
            if (!fileItems.isEmpty()) {
                ...
                UnZip.unZip(is, workingDir, getCommandExecutorService(commandServiceReference));
            }
        } catch (IOException | ServletException e) {
            logger.error("Unable to extract the uploaded archive in {}", workingDir, e);
            reportCommandFailure(resp, isNull(e.getMessage()) ? e.toString() : e.getMessage());
        } finally {
            if (is != null) {
                try {
                    is.close();
    ...
    private void reportCommandFailure(HttpServletResponse resp, String message) throws IOException {
        resp.getWriter().write(GwtSafeHtmlUtils.htmlEscape(message));
    }

    private CommandExecutorService getCommandExecutorService(
            final ServiceReference<PasswordCommandService> commandServiceReference) throws ServletException {
        final Class<? extends CommandExecutorService> executorServiceClass = Boolean.TRUE
                .equals(commandServiceReference.getProperty(COMMAND_PRIVILEGED)) ? PrivilegedExecutorService.class
                        : UnprivilegedExecutorService.class;

        try {
            return ServiceLocator.getInstance().getService(executorServiceClass);
        } catch (GwtKuraException e) {
            throw new ServletException("Error locating the command executor service", e);
        }
    }
